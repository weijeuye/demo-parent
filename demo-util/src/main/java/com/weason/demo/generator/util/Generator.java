...
import com.weason.demo.generator.entity.ResultJson;
...
public class Generator {
	...
	//⑥创建CONTROLLER
	public static ResultJson createController(String url, BasisInfo bi, Boolean flag) {
		if (flag){
			//保证父类一直存在
			createAbstractController( url, bi);
		}
	...
	public static ResultJson createAbstractController(String url,BasisInfo bi) {
		String fileUrl= getGeneratorFileUrl(url, bi.getAbstractControllerUrl(),"Base", CONTROLLER);
		return FreemarkerUtil.createFile(bi, "BaseController.ftl", fileUrl);
	}
